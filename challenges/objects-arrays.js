...
let result = graduates.map(function (a) {
  return universities.push(a.university)
})
universities.sort()
...
const contactInfo = [];
graduates.map(function (b) {
  return contactInfo.push(b.first_name + " " + b.email)
})
...
zooAnimals.forEach(function (item, key) {
...
zooAnimals.map(function (a) {
  lowCaseAnimalNames.push(a.animal_name.toLowerCase())
})
...
zooAnimals.filter(function (a) {
  if (a.population < 5) {
    return lowPopulationAnimals.push(a.animal_name)
  }
